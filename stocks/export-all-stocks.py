...
for name in os.listdir('.'):
    if name.endswith('xls') or name.endswith('xlsx'):
        ...
        if name.startswith('sh'):
            DataframeDict[key]=pd.read_excel(name,  dtype={'A股代码':'str'}, parse_dates=[4,], engine='xlrd')
        elif name.startswith('sz'):
            DataframeDict[key]=pd.read_excel(name, dtype={'A股代码':'str'}, parse_dates=[6,], engine='openpyxl')
...
df_sz00[~df_sz00['A股简称'].str.contains('ST')].to_csv('sz00.txt', columns=['A股代码',], header=None, index=False)
df_sz30[~df_sz30['A股简称'].str.contains('ST')].to_csv('sz30.txt', columns=['A股代码',], header=None, index=False)
df_sh60[~df_sh60['证券简称'].str.contains('ST')].to_csv('sh60.txt', columns=['A股代码',], header=None, index=False)
df_sh68[~df_sh68['证券简称'].str.contains('ST')].to_csv('sh68.txt', columns=['A股代码',], header=None, index=False)
